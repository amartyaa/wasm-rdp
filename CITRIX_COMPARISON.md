...
- Docs-tree audit (§10): product TOC — docs.citrix.com/en-us/citrix-virtual-apps-desktops ; content redirection family (bidirectional, host-to-client, Local App Access) — …/general-content-redirection/bidirectional-content-redirection.html ; …/general-content-redirection/host-to-client-redirection.html ; …/general-content-redirection/laa-url-redirect.html ; session resilience / prelaunch / linger / Workspace Control — …/manage-deployment/sessions.html ; ICA policy settings + virtual-channel allow-list — …/policies/reference/ica-policy-settings.html ; …/virtual-channel-allow-list-policy-settings.html ; printing (UPD/UPS, session printers) — …/policies/reference/ica-policy-settings/printing-policy-settings.html ; …/printing/printing-provision-printers.html ; devices (CDM, serial, TWAIN/WIA, specialty keyboards) — …/devices/client-drive-mapping.html ; …/devices/serial-devices.html ; multimedia redirection (Windows Media, HTML5 video, webcam compression, BCR) — …/multimedia/windows-media-redirection.html ; …/multimedia/html5-multimedia-redirection.html ; …/multimedia/webcam-compression.html ; …/multimedia/browser-content-redirection.html
...
## 10. Appendix B — Citrix docs, section-by-section audit

§4 grouped features by *domain*. This appendix instead walks **Citrix's own documentation structure**
(docs.citrix.com/en-us/citrix-virtual-apps-desktops, CR 2603 tree, verified July 2026) section by
section, so nothing in their table of contents escapes the comparison. Status legend as before
(✅ / 🟡 / 🔴 / ⚪ non-goal). Sections already treated in depth link back to §4.

### 10.1 Platform & lifecycle sections (docs: Start / Plan / Build / Deliver / Upgrade / Manage / Investigate)

| Citrix docs section | What it covers | IronBridge status |
|---|---|---|
| Setup overview, System requirements | Sizing the farm, OS matrices | ⚪ — no farm; one binary, any modern Chromium browser |
| Delivery methods, Design guidance | VDI vs published apps vs RemotePC topologies | 🟡 — two delivery modes exist: full desktop + RAIL app (§4.2); no RemotePC/brokered topologies |
| Machine catalogs / Delivery groups / Application groups | Fleet grouping, assignment, MCS/PVS provisioning | ⚪ — no fleet concept; one `--rdp-target` ([main.rs:35](server/src/main.rs#L35)); §11 adds a dynamic *app* catalog, not a machine catalog |
| StoreFront / Workspace | Resource store, SSO, enumeration | 🔴/🟡 — login page + static app dropdown ([app.js:383](web/app.js#L383)); no per-user enumeration, no SSO (§4.8) |
| Director / Site Analytics / Monitor | Helpdesk console, session drill-down, shadowing, probes | ⚪ — client-side HUD only ([app.js:1388](web/app.js#L1388)); no server-side visibility (the proxy can't see *into* sessions by design — it never parses a PDU) |
| Upgrade & Migrate | Rolling farm upgrades | ⚪ — replace one binary |
| Manage → Sessions (**session resilience settings**) | Session Reliability, Auto Client Reconnect, ICA keep-alive, **reconnection modes** (`DisconnectedOnly`, `SameEndpointOnly`), **Workspace Control** (sessions follow the user across devices), **session prelaunch & linger** (pre-created sessions for instant launch, sessions held after app close) | 🔴 mostly — reconnect exists and is careful (backoff/flap/takeover, [app.js:277-1349](web/app.js#L277)) but there's no server-side session hold (§4.7), no roaming semantics (RDP itself reconnects to the same Windows session — accidental "roaming" — but nothing is *managed*), no prelaunch/linger (contradicts RAIL v1's app-exit-ends-session design, deliberately) |
| Autoscale, Zones, Local Host Cache, Service Continuity | Power management, geo-distribution, broker outage resilience | ⚪ — no broker to cache or scale |
| Licensing | License server, user/device/concurrent models | ⚪ — none |

### 10.2 HDX / Graphics docs sections

| Citrix docs section | What it covers | IronBridge status |
|---|---|---|
| Thinwire | Default adaptive display codec, progressive modes | ✅ equivalent role played by RFX/RFX-Progressive + legacy bitmap (§4.1) |
| GPU acceleration / HDX 3D Pro / OpenGL | Server GPU encode, vGPU workloads | ⚪ — charter is GPU-less hosts; client-GPU decode instead ([session.rs:1394](wasm/src/session.rs#L1394)) |
| Text-based session watermark | Server-drawn traceable overlay (Thinwire-only) | 🔴 — none; client overlay proposed (§7 T2.3) |
| DPI / display scaling | Per-monitor DPI matching | 🔴 — §4.1, §7 T1.4 |
| Framehawk (removed by Citrix) | — | n/a (dead on their side too) |

### 10.3 Multimedia docs sections

| Citrix docs section | What it covers | IronBridge status |
|---|---|---|
| Audio features | Adaptive audio, bidirectional, EDT-lossy, quality enhancer | 🟡 — playback stack strong (§4.4); no mic (§7 T1.1) |
| **Windows Media redirection** | Intercepts host media pipeline, ships *compressed source media* to client for local decode (bandwidth ≪ screen-scraping) | 🔴 — no RDP-side equivalent wired (MS-RDPEV multimedia redirection exists in the protocol but is effectively legacy); AVC420 full-screen encode is the modern substitute — video plays fine, just at screen-encode bandwidth, not source-file bandwidth |
| **HTML5 multimedia redirection** | `HdxVideo.js` injected into web pages redirects `<video>` elements | ⚪ — requires instrumenting remote web pages; poor ROI, same verdict as BCR |
| **Browser content redirection (BCR)** | Whole browser viewport rendered by *client-side* engine | ⚪ — §7 Tier 3 |
| Webcam compression (HDX RealTime) | Client-side H.264 webcam encode over multimedia VC | 🔴/⚪ — §4.4, §7 Tier 3 (defer) |
| Optimization for Teams (classic + SlimCore) | Endpoint media offload | ⚪ — needs native plugin (§4.4) |

### 10.4 General content redirection docs sections

A whole docs area §4 only grazed — now covered:

| Citrix docs section | What it covers | IronBridge status |
|---|---|---|
| Client drive mapping (CDM) | Local drives inside session | 🔴 — §7 T2.1 (File System Access + RDPDR) |
| Generic USB / USB mass storage | Raw USB projection, composite device splitting | ⚪ — browser can't (§4.5) |
| **Host-to-client redirection** | URLs/file-types opened *in session* launch on the *client* (server FTA → client handler) | 🔴 feasible-lite — a RAIL/desktop session could ship URLs to the browser tab (`window.open`); niche but cheap; not roadmapped |
| **Bidirectional content redirection** | URL allow-lists route browsing client↔session both directions | 🔴 — client→session direction would need an agent; session→client is the host-to-client case above |
| **Local App Access (reverse seamless)** | *Local* apps composited into the *remote* desktop | ⚪ — inverse of RAIL; browser cannot composite local windows into a canvas, and the VDA does the server half |

### 10.5 Devices docs sections

| Citrix docs section | What it covers | IronBridge status |
|---|---|---|
| Client drive mapping | (see 10.4) | 🔴 → T2.1 |
| Serial ports (COM) | Legacy device passthrough | ⚪ (Web Serial exists but projecting it into RDP has no channel except RDPDR-serial — no demand, skip) |
| Specialty keyboards (Bloomberg) | Split composite HID handling | ⚪ |
| TWAIN / WIA scanners | In-session scanning | ⚪ |
| Webcams | (see 10.3) | 🔴/⚪ |
| Mobile & touch screen devices | Touch remoting, gestures, mobile UX | 🔴 — §7 T1.7 |

### 10.6 Printing docs sections (deeper than §4.5)

Citrix printing is its own doc tree: **provision printers** (session printers policy, auto-created
client printers, Universal Print Driver EMF/XPS, **Universal Print Server** for network printers via
SOAP), **printing policies** (driver mapping, compatibility lists, bandwidth caps), maintenance.
IronBridge: 🔴 nothing. The browser-realistic slice remains T2.2 (RDPDR print queue → client PDF
print dialog) — i.e., Citrix's "auto-created client printer with UPD" concept, minus everything else.
Their UPS/network-printing tier is ⚪ (server-side infrastructure).

### 10.7 Policies docs sections

Citrix has a full **policy engine**: hundreds of ICA policy settings (audio, bandwidth *per virtual
channel*, clipboard direction/format filtering, drive/USB allow-deny, graphics quality, session
limits/idle timers, **virtual channel allow-list** — default-on since 2109), scoped by user/group/
device/network, authored in Studio, modeled/compared before rollout.

IronBridge's "policy engine" is **~6 boolean server flags** ([main.rs:53-72](server/src/main.rs#L53))
+ client-side localStorage toggles ([app.js:82-343](web/app.js#L82)) — global, not per-user, no
scoping. Structurally: IronBridge policies gate features **client-side or at the proxy**, while
Citrix enforces **at the VDA** (tamper-resistant). What's worth stealing, in order: (1) per-user
scoping once T0.1 auth exists; (2) clipboard *direction* control (today's flags are all-or-nothing
per type, [clipboard.rs:129](wasm/src/clipboard.rs#L129)); (3) session idle timeout. A full policy
engine is ⚪.

### 10.8 Secure docs sections

| Citrix docs section | What it covers | IronBridge status |
|---|---|---|
| Gateway integration, TLS, transport security | Edge auth, E2E TLS | 🔴 — the §4.8 headline gaps (T0.1, T0.3) |
| Smart cards, FAS, FIDO2/WebAuthn | Auth into and inside sessions | 🔴/⚪ — NTLM-only NLA (T1.6); FIDO2-in-session not browser-projectable (§4.5) |
| App Protection | Anti-keylog/capture/DLL-injection, tamper detection | ⚪ kernel-level; watermark T2.3 is the feasible sliver |
| Delegated administration / RBAC | Admin role scoping | ⚪ — no admin plane (until §11's minimal one) |
| Security considerations & best practices | Hardening guidance | 🟡 — systemd unit ships hardening (`NoNewPrivileges`, `ProtectSystem=strict`, [scripts/ironbridge.service](scripts/ironbridge.service)); Windows service runs default privileges; no doc equivalent — worth writing a SECURITY.md once T0.x land |

### 10.9 Monitor / SDK docs sections

Director, session details, logon-duration breakdown, shadowing, probes: ⚪ (no agent = no telemetry
source beyond the wire). SDKs: Citrix ships PowerShell SDKs + a **Virtual Channel SDK** (write custom
VCs — how Teams/Zoom plugins exist). IronBridge's equivalent extension story is "fork IronRDP and add
a channel" — actually *more* open (the whole stack is source), but with zero API stability promises. ⚪
as a product feature; the openness is §5's point 5.

### 10.10 Audit takeaway

Walking Citrix's own docs tree confirms §1's verdict with sharper edges: **every 🔴 that is
browser-feasible already sits in §7's tiers** (auth, frame pacing, mic, IME, resize, HiDPI, RAIL
polish, drives, printing, watermark, touch) — the docs sweep surfaced no *new* feasible gap it
missed, and it hardened several ⚪ verdicts (Local App Access, BCR, HTML5-video redirection, UPS,
serial/TWAIN, policy engine) with concrete mechanism-level reasons. The two genuinely *new* nuances:
(a) Citrix's media redirection family (10.3) is an entire bandwidth-optimization discipline RDP/
IronBridge answers with one blunt instrument (full-screen AVC420) — fine for the charter, worth
knowing; (b) session **prelaunch/linger/roaming** semantics (10.1) are a real UX tier above
"reconnect", and the only piece IronBridge could borrow cheaply is treating disconnect-vs-logoff
distinctly in RAIL mode (today app-exit *ends* the session by design).

---

## 11. Design: publish apps with no flags and no restart

**Problem.** Today the RemoteApp catalog is CLI/env-only: `--app "Name=path"` / `IB_REMOTE_APPS`
([main.rs:71-72](server/src/main.rs#L71)) is parsed once at startup into `AppConfig`
([main.rs:112-119](server/src/main.rs#L112)). Changing the catalog means editing the service
ImagePath (the `CommandLineToArgvW` double-quoting trap that produced opaque error 1053 — see
CLAUDE.md §3) and restarting the service, dropping every live session. This is IronBridge's version
of Citrix's "publish an application" workflow (Studio → app group → instant), and it's the worst
admin experience in the product.

**Key enabling fact (verified):** the injection point is *already per-request*. `embedded_handler`
builds `window.__IB_REMOTE_APPS` fresh on every `index.html` render
([main.rs:224-227](server/src/main.rs#L224)). Only the **source** is frozen. Swap the frozen
`Vec<String>` for a tiny file read at render time and hot publishing falls out — no watcher, no
cache, no signal handling, no restart.

### 11.1 Phase 1 — catalog file, read per request (the whole ask, ~40 lines)

- **File:** `remote-apps.txt` beside the binary (resolved via `std::env::current_exe()`, so the
  Windows service — whose CWD is `system32` — finds it; overridable with optional `--apps-file`).
  Format: **one `Name=path` entry per line**, `#` comments, blank lines ignored — the exact entry
  syntax `remote_apps_json` already parses ([main.rs:182-194](server/src/main.rs#L182)), so no new
  parser, and names/paths with spaces need **no quoting at all** (line-delimited beats both
  `CommandLineToArgvW` and `;`-splitting). Kills the ImagePath-quoting failure class outright.
- **Load rule:** on each `index.html` request, if the file exists and parses to ≥1 entry, it **is**
  the catalog; otherwise fall back to the CLI/env entries (which become a seed/bootstrap mechanism).
  Precedence is absolute, not merged — merge semantics create "where did this entry come from"
  confusion for zero benefit.
- **Enablement:** `enable_remote_app = --enable-remote-app || IB_ENABLE_REMOTE_APP || file present
  with ≥1 entry`. Dropping a one-line file next to the binary both enables RemoteApp *and* publishes
  the first app — literally zero flags, zero restarts. Deleting the file (or all its entries)
  un-publishes and hides the picker on the next page load.
- **Admin workflow:** `notepad remote-apps.txt` → save → user refreshes the login page. That's the
  entire procedure. `install-service-windows.ps1` seeds a commented sample file instead of baking
  the catalog into the ImagePath.
- **Implementation:** `AppConfig` keeps the CLI values; `embedded_handler` calls a new
  `load_remote_apps(&config) -> (bool, Vec<String>)` that does the file read + fallback. One
  function, one call site, no new dependencies, no state.

**Per the CLAUDE.md rule:**
- *Windows (headless, GPU-less):* full fit — file beside `server.exe`, service never restarts; the
  1053 quoting saga becomes irrelevant. *Ubuntu + GRD / xRDP:* the catalog UI is RAIL-gated and RAIL
  is Windows-hosts-only, so behavior is unchanged there; the same file mechanism trivially extends to
  other hot-reloadable settings later if wanted.
- *Performance:* one ≤4 KB file read per **login-page render** — not per frame, not per WS message.
  Zero datapath impact. No inotify/watcher thread to babysit.
- *Compatibility:* wire-format untouched (catalog is UI-only; the host still gates launches via
  `TSAppAllowList`); `?app=` deep links, localStorage persistence, reconnect (app string rides
  `savedCredentials`), multimon/audio/clipboard all unaffected.
- *Simplicity:* strictly less machinery than any alternative (SIGHUP reload, admin API, config DB) —
  the read-per-request trick exploits the injection point that already exists.

**Limits (stated):** publishing still doesn't touch the *host* — with `fDisabledAllowList=1` any
path launches and the catalog is pure UI; in locked-down mode the admin still registers the app in
`TSAppAllowList` on the host (no pre-session channel exists to do it remotely — same chicken-and-egg
documented in the original RAIL plan). Already-connected sessions keep their current app (catalog is
read at login).

### 11.2 Phase 2 (optional, needs T0.1 auth first) — admin edit UI

A `GET/POST /api/admin/apps` pair that lists/rewrites `remote-apps.txt`, plus a minimal `/admin`
page — gated by the T0.1 auth layer (an unauthenticated admin endpoint would be a worse hole than
editing a file over RDP). Value-add over Phase 1 is convenience only; Phase 1 fully delivers
"no flags, no restart". Build it when the auth story exists, not before.

**Citrix parity note:** Phase 1 ≈ Studio's "publish app" (minus per-user assignment, which arrives
with T0.1 auth + T2.4 per-user catalogs); Phase 2 ≈ a one-page Studio. That is the right amount of
Citrix to copy.
